...
class TCBinaryProtocol(TProtocolBase):
    ...
    def writeMessageBegin(self, name, type, seqid):
        if self.strictWrite:
            self.writeI32(TCBinaryProtocol.VERSION_1 | type)
    ...
    def readMessageBegin(self):
        sz = self.readI32()
        if sz < 0:
            version = sz & TCBinaryProtocol.VERSION_MASK
            if version != TCBinaryProtocol.VERSION_1:
                raise TProtocolException(
                    type=TProtocolException.BAD_VERSION,
                    message='Bad version in readMessageBegin: %d' % (sz))
            type = sz & TCBinaryProtocol.TYPE_MASK
...
class TCBinaryProtocolFactory(object):
    ...
    def getProtocol(self, trans):
        prot = TCBinaryProtocol(trans, self.strictRead, self.strictWrite,
                               string_length_limit=self.string_length_limit,
                               container_length_limit=self.container_length_limit)
        return prot


class TCBinaryProtocolAccelerated(TCBinaryProtocol):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(TCBinaryProtocolAccelerated, self).__init__(*args, **kwargs)
        try:
...
class TCBinaryProtocolAcceleratedFactory(object):
    def __init__(self,
    ...
    def getProtocol(self, trans):
        return TCBinaryProtocolAccelerated(
            trans,
            string_length_limit=self.string_length_limit,
            container_length_limit=self.container_length_limit,
